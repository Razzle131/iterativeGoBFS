...
func array_contains(arr [][3]int, val [3]int) bool {
	for i := 0; i < len(arr); i++ {
...
func Solution(inputData [][3]int) {
	...
	for i := 0; i < len(inputData); i++ {
		if array_contains(visited, inputData[i]) {
			continue
		}
		...
		for len(queue) > 0 {
			...
			for j := 0; j < 4; j++ {
				...
				if isValid(next_node) && board[next_node[1]][next_node[0]] == next_node[2] && !array_contains(visited, next_node) {
